...
extern __managed__ int First10KPrimes[];
extern __managed__ int First50KPrimes[];
 ...
 */
__device__ __forceinline__ bool spanrowQ( uint row, uint n ) {
	uint ret;
...
template<> __device__ __forceinline__ ulong mod<ulong>( ulong x, ulong divisor ) {
	ulong ret;
...
__host__ CUDART_DEVICE  int largestMutualFactor(uint v, uint w, bool smallest=false);
