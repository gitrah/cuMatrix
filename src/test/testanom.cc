...
template int testAnomDet<ulong>::operator()(int argc, char const ** args) const;
...
*/
	CuMatrix<T> pdens(x.m,x.n,false,true);
	CuMatrix<T> pvec(x.m,1,false,true);
	...
	xval.syncBuffers();
	CuMatrix<T> pdensval(xval.m,xval.n,false,true);
...
template int testMVAnomDet<float>::operator()(int argc, char const ** args) const;
template int testMVAnomDet<double>::operator()(int argc, char const ** args) const;
template<typename T> int testMVAnomDet<T>::operator()(int argc, const char** args) const {
	outln( "opening " << ANOMDET_SAMPLES2_FILE);
	map<string, CuMatrix<T>*> results = util<T>::parseOctaveDataFile(ANOMDET_SAMPLES2_FILE,false, true);
...
template int testRedWineScSv<float>::operator()(int argc, char const ** args) const;
template int testRedWineScSv<double>::operator()(int argc, char const ** args) const;
template<typename T> int testRedWineScSv<T>::operator()(int argc, const char** args) const {
	outln( "opening " << REDWINE_SAMPLES_FILE);
	map<string, CuMatrix<T>*> results = util<T>::parseCsvDataFile(REDWINE_SAMPLES_FILE,";",false, true,true);
...
template int testCsv<float>::operator()(int argc, char const ** args) const;
template int testCsv<double>::operator()(int argc, char const ** args) const;
template<typename T> int testCsv<T>::operator()(int argc, const char** args) const {
	string path = b_util::getPath(argc,args,REDWINE_SAMPLES_FILE);
	outln( "opening " << path);
	map<string, CuMatrix<T>*> results = util<T>::parseCsvDataFile(path.c_str(),",",false, true, false);
